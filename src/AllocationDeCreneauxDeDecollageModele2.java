...
public class AllocationDeCreneauxDeDecollageModele2 {
    ...
    public static void SolveProblem(ArrayList<Vol> vols) {
        ...
        for(Map.Entry m:lesintervallesGotten.entrySet()){
            ...
            for (int j = 0; j < lesintervallesGotten.get(m.getKey()).size(); j++) {
                Constraint c1 = model.arithm(dij[dij_index-1], "<", lesintervallesGotten.get(m.getKey()).get(j).lb);
                Constraint c2 = model.arithm(dij[dij_index-1], ">", lesintervallesGotten.get(m.getKey()).get(j).ub);
